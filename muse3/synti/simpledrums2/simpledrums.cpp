...
bool SimpleSynth::setController(int channel, int id, int val)
{
   ...
   if (id >= SS_FIRST_CHANNEL_CONTROLLER && id <= SS_LAST_CHANNEL_CONTROLLER ) {
      ...
      case SS_CHANNEL_CTRL_ROUTE:
         if (SS_DEBUG_MIDI)
            printf("Received channel ctrl pitch %d for channel %d\n", val, ch);
         channels[ch].route = (SS_ChannelRoute)val;
         printf("SS_CHANNEL_CTRL_ROUTE %d\n", channels[ch].route);
   ...
   if (synth_state == SS_RUNNING) {
      ...
      for (int ch=0; ch < SS_NR_OF_CHANNELS; ch++) {
         ...
         if (channels[ch].state == SS_SAMPLE_PLAYING) {
            ...
            for (int i=0; i<len; i++) {
               if(channels[ch].route == SS_CHN_ROUTE_MIX) {
                  out[0][i+offset]+=processBuffer[0][i];
                  out[1][i+offset]+=processBuffer[1][i];
               }
               //copy data for individual channels
...
void SimpleSynth::getInitData(int* n, const unsigned char** data) 
{
   ...
   for (int ch=0; ch<SS_NR_OF_CHANNELS; ch++) {
      ...
      initBuffer[i+9] = channels[ch].route;
      ...
      if (channels[ch].sample) {
         ...
         initBuffer[i+10] = (byte) filenamelen;
         memcpy((initBuffer+(i+11)), channels[ch].sample->filename.c_str(), filenamelen);
         if (SS_DEBUG_INIT) {
            printf("initBuffer[%d] - filenamelen: %d\n", i+10, filenamelen);
            printf("initBuffer[%d] - initBuffer[%d] - filename: ", (i+11), (i+11) + filenamelen - 1);
            for (int j = i+11; j< i+11+filenamelen; j++) {
...
void SimpleSynth::parseInitData(const unsigned char* data)
{
   ...
   for (int ch=0; ch<SS_NR_OF_CHANNELS; ch++) {
      ...
      if(dataVersion > 2) {
         channels[ch].route = (SS_ChannelRoute)*(ptr);
         guiUpdateRoute(ch, *(ptr));
         ptr++;
      }
...
void SimpleSynth::guiUpdateRoute(int ch, int val)
{
   SS_TRACE_IN
         MusECore::MidiPlayEvent ev(0, 0, ch, MusECore::ME_CONTROLLER, SS_CHANNEL_ROUTE_CONTROLLER(ch), val);
   gui->writeEvent(ev);
   SS_TRACE_OUT
}
