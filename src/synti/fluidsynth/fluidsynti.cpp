...
void FluidSynth::getInitData(int* n, const unsigned char** data) 
      ...
      for (std::list<FluidSoundFont>::const_iterator it =stack.begin(); it!=stack.end(); it++) {
            ...
            memcpy(chptr, it->file_name.toUtf8().constData()+offset, it->file_name.size()-offset+1);
...
{
    printf("projPathPtr ");
    std::cout << projPathPtr.toLocal8Bit().constData() << std::endl;
    ...
    for (int i=0; i<nr_of_fonts; i++) {
        fonts[i].file_name = QString((char*)&d[arrayIndex]);
        arrayIndex+=fonts[i].file_name.size()+1;
        QByteArray ba = projPathPtr.toUtf8();
        ...
        std::cout << "SOUNDFONT FILENAME + PATH " << fonts[i].file_name.toLocal8Bit().constData() << std::endl;
    }
...
// Deprecated functions at version >= 2.2
    ...
    for (int i=0; i<nrOfSoundfonts; i++) {
        pushSoundfont(fonts[i].file_name.toUtf8().constData(), fonts[i].extid);
    }
    delete[] fonts;
}
...
bool FluidSynth::pushSoundfont (const char* filename, int extid)
      ...
      else
      {
          ...
          //printf("current path: %s \nmuseProject %s\nfilename %s\n",QDir::currentPath().toLocal8Bit().constData(), MusEGlobal::museProject.toLocal8Bit().constData(), filename);
          QFileInfo fi(fn);
...
void FluidSynth::sendSoundFontData()
      ...
      *(ndata + 1) = (unsigned char)stack.size (); //Nr of Soundfonts
      ...
      for (std::list<FluidSoundFont>::iterator it = stack.begin(); it != stack.end();  ++it) {
            ...
            memcpy(chunk_start, it->name.toUtf8().constData(), name_len); //First, store the fontname
...
void FluidSynth::dumpInfo()
      ...
      printf("\n");
      for (std::list<FluidSoundFont>::iterator it = stack.begin(); it != stack.end(); it++)
            printf("Font: %s\tintid: %d\textid %d\tfilename:%s\n", it->name.toLocal8Bit().constData(), it->intid, it->extid, it->file_name.toLocal8Bit().constData());
...
void LoadFontWorker::execLoadFont(void * t)
{
      ...
      QByteArray ba = h->file_name.toUtf8();
