...
void ScoreCanvas::mousePressEvent (QMouseEvent* event)
{
    ...
    list<staff_t>::iterator staff_it=staff_at_y(qRound(event->position().y()) + y_pos);

    int y=qRound(event->position().y()) + y_pos - staff_it->y_draw;
    int x=qRound(event->position().x())+x_pos-x_left;
    int tick=flo_quantize_floor(x_to_tick(x), quant_ticks());

    if (staff_it!=staves.end())
    {
        if (qRound(event->position().x()) <= x_left) //clicked in the preamble?
        {
...
void ScoreCanvas::mouseReleaseEvent (QMouseEvent* event)
{
    ...
    {
        int y=qRound(event->position().y())+y_pos;
...
void ScoreCanvas::mouseMoveEvent (QMouseEvent* event)
{
    ...
    if (dragging)
    {
        int dx=qRound(event->position().x())-mouse_down_pos.x();
        int dy=qRound(event->position().y())-mouse_down_pos.y();
        ...
        {
            int win_x=qRound(event->position().x());
    ...
    {
        int win_y=qRound(event->position().y());
