...
void Handle::mouseMoveEvent(QMouseEvent* ev)
      {
      rootWin->move(qRound(ev->globalPosition().x())-dx, qRound(ev->globalPosition().y()) - dy);
      }
...
void Handle::mousePressEvent(QMouseEvent* ev)
      {
      rootWin->raise();
      dx = qRound(ev->globalPosition().x()) - rootWin->x();
      dy = qRound(ev->globalPosition().y()) - rootWin->y();
      }
