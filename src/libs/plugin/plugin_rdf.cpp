  ...
  if(pluginDir.exists())
  {
    ...
    while(it != fi_list.end())
    {
      ...
      else
      {
        ...
        for( ; i < rdfs_sz; ++i)
        {
          // Qt6: QFileInfo(const QString&) became an explicit constructor,
          //  so copy-initialization ("= ...") no longer compiles here.
...
void scanLrdfPlugins(QStringList* rdfs, bool debugStdErr)
{
    ...
    if(!lrdfPath.isEmpty())
    {
        // QString::*EmptyParts is gone in Qt6; Qt::SkipEmptyParts is required now.
