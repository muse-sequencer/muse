...
EditInstrument::EditInstrument(QWidget* parent, const char* name, WFlags fl)
   : EditInstrumentBase(parent, name, fl)
      {
      patchpopup = new QPopupMenu(patchButton);
...
//        workingInstrument.assign( *wip );
      ...
      connect(listController, SIGNAL(doubleClicked(QListBoxItem*)), SLOT(addControllerClicked()));
        ...
        for(iMidiInstrument imi = midiInstruments.begin(); imi != midiInstruments.end(); ++imi) 
        {
          ...
          if((*imi)->iname() == s) 
          {
            ...
            if(s != workingInstrument.iname() && !(*imi)->filePath().isEmpty())
            {
              ...
              oldMidiInstrument = (ListBoxData*)instrumentList->findItem(s, ExactMatch); 
...
void EditInstrument::patchButtonClicked()
{
      ...
      if (pg->size() > 1) {
            for (ciPatchGroup i = pg->begin(); i != pg->end(); ++i) {
                  PatchGroup* pgp = *i;
                  QPopupMenu* pm = new QPopupMenu(patchpopup);
