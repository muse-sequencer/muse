...
LMaster::LMaster()
   : MidiEditor(0, 0, 0)
      ...
      qa->connectItem(qa->insertItem(CTRL+Key_Y), song, SLOT(redo()));
...
//      mainGrid->addWidget(corner,  1, 1, AlignBottom | AlignRight);
      ...
      connect(view, SIGNAL(pressed(QListViewItem*, const QPoint&, int)), SLOT(itemPressed(QListViewItem*, const QPoint&, int)));
      connect(view, SIGNAL(doubleClicked(QListViewItem* )), SLOT(itemDoubleClicked(QListViewItem*)));
 ...
 */
void LMaster::itemPressed(QListViewItem* i, const QPoint& /*p*/, int column)
      {
...
//---------------------------------------------------------
void LMaster::itemDoubleClicked(QListViewItem* i)
      {
      ...
      //
            ...
            if (newtick == 0) { // Do not allow change of position to beginning of song
                  ...
                  if (editedItem->getType() == LMASTER_TEMPO) {
                        ...
                        song->endUndo(SC_TEMPO);
                        // Select the item:
                        QListViewItem* newSelected = (QListViewItem*) getItemAtPos(newtick, LMASTER_TEMPO);
                        if (newSelected) {
                              view->clearSelection();
                        ...
                        // Select the item:
                        QListViewItem* newSelected = (QListViewItem*) getItemAtPos(newtick, LMASTER_SIGEVENT);
                        if (newSelected) {
                              view->clearSelection();
                              view->setSelected(newSelected, true);
                              }
...
//---------------------------------------------------------
LMasterTempoItem::LMasterTempoItem(QListView* parent, const TEvent* ev)
...
//---------------------------------------------------------
LMasterSigEventItem::LMasterSigEventItem(QListView* parent, const SigEvent* ev)
      : LMasterLViewItem(parent)
      {
      sigEvent = ev;
      unsigned t = ev->tick;
...
void LMaster::tempoButtonClicked()
      ...
      new LMasterTempoItem(view, ev);
      QListViewItem* newTempoItem = view->firstChild();
...
void LMaster::timeSigButtonClicked()
      ...
      new LMasterSigEventItem(view, ev);
      QListViewItem* newSigItem = view->firstChild();
