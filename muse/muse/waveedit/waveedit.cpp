...
WaveEdit::WaveEdit(PartList* pl)
   : MidiEditor(1, 1, pl)
      ...
      select->insertItem(QIconSet(*select_allIcon), tr("Select &All"),  CMD_SELECT_ALL);
      select->insertItem(QIconSet(*select_deselect_allIcon), tr("&Deselect All"), CMD_SELECT_NONE);

      menuEdit->insertItem(QIconSet(*selectIcon), tr("Select"), select);
