...
SS_PluginChooser::SS_PluginChooser(QWidget* parent, const char* name)
      ...
      for (iPlugin i=plugins.begin(); i !=plugins.end(); i++) {
            ...
            if ( ((*i)->outports() == 2 || (*i)->outports() == 1) && ((*i)->inports() == 2 || (*i)->inports() == 1) ) {
                  QListViewItem* tmpItem = new QListViewItem(effectsListView);
      ...
      connect(effectsListView, SIGNAL(selectionChanged(QListViewItem*)), SLOT(selectionChanged(QListViewItem*)));
      connect(effectsListView, SIGNAL(doubleClicked(QListViewItem*)), SLOT(doubleClicked(QListViewItem*)));
      SS_TRACE_OUT
      }
 ...
 */
void SS_PluginChooser::selectionChanged(QListViewItem* item)
      {
      SS_TRACE_IN
      selectedItem  = item;
      SS_TRACE_OUT
 ...
 */
void SS_PluginChooser::doubleClicked(QListViewItem* /*item*/)
      {
      SS_TRACE_IN
      selectedPlugin = findSelectedPlugin();
      SS_TRACE_OUT
...
SS_PluginFront::SS_PluginFront(QWidget* parent, int in_fxid, const char* name)
      : QGroupBox(parent, name), fxid (in_fxid)
      {
      SS_TRACE_IN
      expanded = false;
      pluginChooser = 0;
      ...
      bigLayout->setResizeMode(QLayout::FreeResize);

      layout = new QHBoxLayout(bigLayout);
      layout->setAlignment(Qt::AlignVCenter);
      layout->setResizeMode(QLayout::FreeResize);


      QVBoxLayout* onOffLayout = new QVBoxLayout(layout);
...
void SS_PluginFront::createPluginParameters()
      {
      SS_TRACE_IN
      expGroup = new QButtonGroup(this);
      ...
      expGroupLayout->setResizeMode(QLayout::FreeResize);
