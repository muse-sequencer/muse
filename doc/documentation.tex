...
\emph{http://www.muse-sequencer.org} is MusE's home on the internet where
...
\textbf{CTRL} refers to the control key on the keyboard, e.g. \textbf{CTRL+C}
...
\textbf{SHIFT} refers to the shift key on the keyboard, see above for usage\\
\textbf{ALT} refers to the alt key on the keyboard, see above for usage\\
\textbf{\$>} is used as a generic definition for a terminal prompt. When the
manual lists a command that shall be typed, the prompt is not part of the
command.\\
Keys are always referred to in bold uppercase, e.g. \textbf{A}. For instance
\textbf{SHIFT+A} for the key a pressed together with the shift key.\\
...
terminal:\\
\hspace*{1cm}\texttt{\$> jackd -d alsa -d hw:0 -p 256}\\
...
on the homepage: http://muse-sequencer.org/index.php/Installation
...
\item FFADO.org for firewire devices.
\end {itemize}
...
server http://jackaudio.org. Make sure jack is installed (if MusE was
...
\begin{figure}[htp]
\centering
\includegraphics[width=.6\textwidth]{pics/no_audio}
...
~\ref{Main/Arranger}) and select \textbf{Add Synth > MESS > Organ}. A Soft
Synth track called Organ-0 should appear as well as a separate GUI for
thesynthesizer.\\
Now right click once more in the Track-pane and select \textbf{Add Midi
...
\includegraphics[width=0.03\textwidth]{pics/arrow_tool}
%\hrulefill
\end{wrapfigure}
Now select the drawing tool icon
from the toolbar, alternatively press the shortcut key \textbf{D}.
...
\begin{figure}[htp]
\centering
\includegraphics[width=.4\textwidth]{pics/organ_synth}
...
\begin{figure}[ht]
\centering
\includegraphics[width=.7\textwidth]{pics/main_window_with_midi_editor}
...
\begin{figure}[htp]
\centering
\includegraphics[width=.7\textwidth]{pics/main_window_annotated}
...
\begin{figure}[htp]
\centering
\includegraphics[width=.7\textwidth]{pics/main_window_with_arrangement}
...
in \texttt{AudioTrack::\_controller}, which is a \texttt{CtrlListList},  % "design decisions" -> "automation"
...
\texttt{AudioTrack::addController} and \texttt{removeController} are used % TODO: swapControllerIDX??
