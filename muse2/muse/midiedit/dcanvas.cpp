...
DCItem::DCItem(const MusECore::Event& e, MusECore::Part* p, int instr) : MCItem(e, p)
      {
      int y  = instr * TH + TH/2;
...
CItem* DrumCanvas::addItem(MusECore::Part* part, MusECore::Event& event)
      ...
      DCItem* ev = new DCItem(event, part, instr);
...
// =======
				...
				if (moveItem(operations, dcitem, newpos, dtype) == false) //error?
        {
...
bool DrumCanvas::moveItem(MusECore::Undo& operations, CItem* item, const QPoint& pos, DragType dtype)
	...
	return false;
      ...
      if (!instrument_map[nheight].tracks.contains(dest_part->track()))
      {
        ...
        if (!instrument_map[nheight].tracks.contains(_curPart->track()))
        {
          ...
          dest_part=(MusECore::MidiPart*)_curPart;
      }
...
CItem* DrumCanvas::newItem(int tick, int instrument, int velocity)
{
  ...
  if (!old_style_drummap_mode && !instrument_map[instrument].tracks.contains(_curPart->track()))
  {
    if (debugMsg)
  ...
  return new DCItem(e, _curPart, instrument);
}
...
void DrumCanvas::keyPressed(int index, int velocity)
{
      ...
      {
        if ( _curPart && instrument_map[index].tracks.contains(_curPart->track()) )
            steprec->record(_curPart,instrument_map[index].pitch,ourDrumMap[index].len,editor->raster(),velocity,MusEGlobal::globalKeyState&Qt::ControlModifier,MusEGlobal::globalKeyState&Qt::ShiftModifier, -1 /* invalid pitch as "really played" -> the "insert rest" feature is never triggered */);
        else
        {
...
MusECore::Event *DrumCanvas::getEventAtCursorPos()
{
    if (_tool != CursorTool)
      return 0;
    if (instrument_map[cursorPos.y()].tracks.contains(_curPart->track()))
    {
...
void DrumCanvas::midiNote(int pitch, int velo)
{
      ...
      if (_midiin && _steprec && !MusEGlobal::audio->isPlaying() && velo && !(MusEGlobal::globalKeyState & Qt::AltModifier) /* && pos[0] >= start_tick && pos[0] < end_tick [removed by flo93: this is handled in steprec->record()] */ )
      {
         ...
         {
           if (_curPart)
             steprec->record(_curPart,0xdead,0xbeef,editor->raster(),velo,MusEGlobal::globalKeyState&Qt::ControlModifier,MusEGlobal::globalKeyState&Qt::ShiftModifier, pitch);
         }
         else
         {
            ...
            {
              if ( instrument_map[i].tracks.contains(_curPart->track()) && ourDrumMap[i].enote==pitch)
              {
                rec_part=_curPart;
                rec_index=i;
                break;
