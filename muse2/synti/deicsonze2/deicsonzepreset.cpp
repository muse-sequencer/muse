...
// writeSet
//---------------------------------------------------------
void Set::writeSet(Xml* xml, bool onlyUsed) {
    xml->tag(0, "deicsOnzeSet version=\"1.0\"");
    xml->strTag(SET_LEVEL, "setName", _setName.c_str());
    for(std::vector<Category*>::iterator i = _categoryVector.begin();
        i != _categoryVector.end(); i++) (*i)->writeCategory(xml, onlyUsed);
    xml->etag(0, "deicsOnzeSet");
}
...
void Category::readCategory(QDomNode catNode) {
	...
	if (catEl.tagName() == "categoryName")
	    _categoryName=catEl.text().ascii();
...
// writeCategory
//---------------------------------------------------------
void Category::writeCategory(Xml* xml, bool onlyUsed) {
    if((!onlyUsed || _isUsed)) {
        xml->tag(SET_LEVEL, "deicsOnzeCategory version=\"1.0\"");
        xml->strTag(CAT_LEVEL, "categoryName", _categoryName.c_str());
        xml->intTag(CAT_LEVEL, "hbank", _hbank);
        for(std::vector<Subcategory*>::iterator i=_subcategoryVector.begin();
            i!=_subcategoryVector.end(); i++)
            (*i)->writeSubcategory(xml, onlyUsed);
        xml->etag(SET_LEVEL, "deicsOnzeCategory");
    }
}
...
void Subcategory::readSubcategory(QDomNode subNode) {
	...
	if (subEl.tagName() == "subcategoryName")
	    _subcategoryName=subEl.text().ascii();
...
// writeSubcategory
//---------------------------------------------------------
void Subcategory::writeSubcategory(Xml* xml, bool onlyUsed) {
    if((!onlyUsed || _isUsed)) {
        xml->tag(CAT_LEVEL, "deicsOnzeSubcategory version=\"1.0\"");
        xml->strTag(SUB_LEVEL, "subcategoryName", _subcategoryName.c_str());
        xml->intTag(SUB_LEVEL, "lbank", _lbank);
        for(std::vector<Preset*>::iterator i=_presetVector.begin();
            i!=_presetVector.end(); i++) (*i)->writePreset(xml, onlyUsed);
        xml->etag(CAT_LEVEL, "deicsOnzeSubcategory");
    }
}
...
// witePreset
//---------------------------------------------------------
void Preset::writePreset(Xml* xml, bool onlyUsed) {
    char s[MAXCHARTAG];
    if((!onlyUsed || _isUsed)) {
        xml->tag(SUB_LEVEL, "deicsOnzePreset version=\"1.0\"");
	
        //algorithm
        xml->strTag(PRE_LEVEL, ALGSTR, (algorithm==FIRST? "FIRST":
                                           ...
                                           (algorithm==FIFTH? "FIFTH":
                                            (algorithm==SIXTH? "SIXTH":
                                             (algorithm==SEVENTH? "SEVENTH":
                                              "EIGHTH"))))))));
        //feedback
        xml->uintTag(PRE_LEVEL, FEEDBACKSTR, feedback);
        //quick edit
        ...
        //lfo
        xml->tag(PRE_LEVEL, "lfo");
        xml->strTag(PRE_LEVEL + 1, WAVESTR, 
                    (lfo.wave==SAWUP? "SAWUP":
                     (lfo.wave==SQUARE? "SQUARE":
                      (lfo.wave==TRIANGL? "TRIANGL":"SHOLD"))));
        xml->uintTag(PRE_LEVEL + 1, SPEEDSTR, lfo.speed);
        xml->uintTag(PRE_LEVEL + 1, DELAYSTR, lfo.delay);
        xml->uintTag(PRE_LEVEL + 1, PMODDEPTHSTR, lfo.pModDepth);
        xml->uintTag(PRE_LEVEL + 1, AMODDEPTHSTR, lfo.aModDepth);
        xml->strTag(PRE_LEVEL + 1, SYNCSTR, (lfo.sync==true? "on":"off"));
        xml->etag(PRE_LEVEL, "lfo");
        //sensitivity
        xml->tag(PRE_LEVEL, "sensitivity");
        xml->uintTag(PRE_LEVEL + 1, PMODSENSSTR, sensitivity.pitch);
        xml->uintTag(PRE_LEVEL + 1, AMSSTR, sensitivity.amplitude);
        for(int i=0; i<NBROP; i++) {
            sprintf(s, AMESTR "%d",i+1);
            xml->strTag(PRE_LEVEL + 1, s, 
                        (sensitivity.ampOn[i]==true? "on":"off"));
        }
        for(int i=0; i<NBROP; i++) {
            sprintf(s,EBSSTR "%d",i+1);
            xml->uintTag(PRE_LEVEL + 1, s, sensitivity.egBias[i]);
        }
        for(int i=0; i<NBROP; i++) {
            sprintf(s, KVSSTR "%d",i+1);
            xml->uintTag(PRE_LEVEL + 1, s, sensitivity.keyVelocity[i]);
        }
        xml->etag(PRE_LEVEL, "sensitivity");
        //frequency
        xml->tag(PRE_LEVEL, "frequency");
        for(int i=0; i<NBROP; i++) {
            sprintf(s, RATIOSTR "%d",i+1);
            xml->doubleTag(PRE_LEVEL + 1, s, frequency[i].ratio);
        }
        for(int i=0; i<NBROP; i++) {
            sprintf(s, FIXSTR "%d",i+1);
            xml->strTag(PRE_LEVEL + 1, s, (frequency[i].isFix==true? "yes":"no"));
        }
        for(int i=0; i<NBROP; i++) {
            sprintf(s, FIXRANGESTR "%d",i+1);
            xml->doubleTag(PRE_LEVEL + 1, s, frequency[i].freq);
        }
        xml->etag(PRE_LEVEL, "frequency");
        //oscWave
        for(int i=0; i<NBROP; i++) {
            sprintf(s, OSWSTR "%d",i+1);
            xml->strTag(PRE_LEVEL, s, (oscWave[i]==W1?"W1":
                                       (oscWave[i]==W2?"W2":
                                        (oscWave[i]==W3?"W3":
                                         (oscWave[i]==W4?"W4":
                                          (oscWave[i]==W5?"W5":
        ...
        for(int i=0; i<NBROP; i++) {
            ...
            xml->intTag(PRE_LEVEL, s, detune[i]);
        }
        //eg
        xml->tag(PRE_LEVEL, "eg");
        for(int i=0; i<NBROP; i++) {
            sprintf(s, ARSTR "%d",i+1);
            xml->uintTag(PRE_LEVEL + 1, s, eg[i].ar);
        }
        for(int i=0; i<NBROP; i++) {
            sprintf(s, D1RSTR "%d",i+1);
            xml->uintTag(PRE_LEVEL + 1, s, eg[i].d1r);
        }
        for(int i=0; i<NBROP; i++) {
            sprintf(s, D1LSTR "%d",i+1);
            xml->uintTag(PRE_LEVEL + 1, s, eg[i].d1l);
        }
        for(int i=0; i<NBROP; i++) {
            sprintf(s, D2RSTR "%d",i+1);
            xml->uintTag(PRE_LEVEL + 1, s, eg[i].d2r);
        }
        for(int i=0; i<NBROP; i++) {
            sprintf(s, RRSTR "%d",i+1);
            xml->uintTag(PRE_LEVEL + 1, s, eg[i].rr);
        }
        for(int i=0; i<NBROP; i++) {
            sprintf(s, SHFTSTR "%d",i+1);
            xml->strTag(PRE_LEVEL + 1, s, (eg[i].egShift==VOF?"VOF":
                                           (eg[i].egShift==V48?"V48":
                                            (eg[i].egShift==V24?"V24":"V12"))));
        }
        xml->etag(PRE_LEVEL, "eg");
        //pitchEg
        xml->tag(PRE_LEVEL, "pitchEg");
        xml->uintTag(PRE_LEVEL + 1, PR1STR,pitchEg.pr1);
        xml->uintTag(PRE_LEVEL + 1, PR2STR,pitchEg.pr2);
        xml->uintTag(PRE_LEVEL + 1, PR3STR,pitchEg.pr3);
        xml->uintTag(PRE_LEVEL + 1, PL1STR,pitchEg.pl1);
        xml->uintTag(PRE_LEVEL + 1, PL2STR,pitchEg.pl2);
        xml->uintTag(PRE_LEVEL + 1, PL3STR,pitchEg.pl3);
        xml->etag(PRE_LEVEL, "pitchEg");
        //outLevel
        for(int i=0; i<NBROP; i++) {
            sprintf(s, OUTSTR "%d",i+1);
            xml->uintTag(PRE_LEVEL, s, outLevel[i]);
        }
        //scaling
        xml->tag(PRE_LEVEL, "scaling");
        for(int i=0; i<NBROP; i++) {
            sprintf(s, RSSTR "%d",i+1);
            xml->uintTag(PRE_LEVEL + 1, s, scaling.rate[i]);
        }
        for(int i=0; i<NBROP; i++) {
            sprintf(s, LSSTR "%d",i+1);
            xml->uintTag(PRE_LEVEL + 1, s, scaling.level[i]);
        }
        xml->etag(PRE_LEVEL, "scaling");
        //function
        xml->tag(PRE_LEVEL, "function");
        xml->intTag(PRE_LEVEL + 1, TRANSPOSESTR, function.transpose);
        xml->strTag(PRE_LEVEL + 1, POLYMODESTR, 
                    (function.mode==POLY? "POLY":"MONO"));
        xml->uintTag(PRE_LEVEL + 1, PBENDRANGESTR, function.pBendRange);
        xml->strTag(PRE_LEVEL + 1, PORTAMODESTR,
                    (function.portamento==FINGER? "FINGER":"FULL"));
        xml->uintTag(PRE_LEVEL + 1, PORTATIMESTR, function.portamentoTime);
        xml->strTag(PRE_LEVEL + 1, FSWSTR, (function.footSw==POR? "POR":"SUS"));
        xml->uintTag(PRE_LEVEL + 1, FCVOLUMESTR, function.fcVolume);
        xml->uintTag(PRE_LEVEL + 1, FCPITCHSTR, function.fcPitch);
        xml->uintTag(PRE_LEVEL + 1, FCAMPLITUDESTR, function.fcAmplitude);
        xml->uintTag(PRE_LEVEL + 1, MWPITCHSTR, function.mwPitch);
        xml->uintTag(PRE_LEVEL + 1, MWAMPLITUDESTR, function.mwAmplitude);
        xml->uintTag(PRE_LEVEL + 1, BCPITCHSTR, function.bcPitch);
        xml->uintTag(PRE_LEVEL + 1, BCAMPLITUDESTR, function.bcAmplitude);
        xml->intTag(PRE_LEVEL + 1, BCPITCHBIASSTR, function.bcPitchBias);
        xml->uintTag(PRE_LEVEL + 1, BCEGBIASSTR, function.bcEgBias);
        xml->uintTag(PRE_LEVEL + 1, ATPITCHSTR, function.atPitch);
        xml->uintTag(PRE_LEVEL + 1, ATAMPLITUDESTR, function.atAmplitude);
        xml->intTag(PRE_LEVEL + 1, ATPITCHBIASSTR, function.atPitchBias);
        xml->uintTag(PRE_LEVEL + 1, ATEGBIASSTR, function.atEgBias);
        xml->intTag(PRE_LEVEL + 1, REVERBRATESTR, function.reverbRate);
        xml->etag(PRE_LEVEL, "function");
        //globalDetune
        //xml->tag("globalDetune", globalDetune);
        //preset name
        xml->strTag(PRE_LEVEL, "name", name.c_str());
        //bank prog
        xml->intTag(PRE_LEVEL, "prog", prog);
        
        xml->etag(SUB_LEVEL, "deicsOnzePreset");
    }
}
